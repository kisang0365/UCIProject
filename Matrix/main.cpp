...
/*	Tree tree;
	...
	*/
	...
	
	{
		...
		//make Mat form.
		cv::Mat newM(3, sz, CV_8UC3, cv::Scalar(0));
		int n = 0;
			...
			if (loadTree.tree.at(a).allBlack == true){
					...
					for (unsigned int i = 0; i < loadTree.tree.at(a).h; i++){
						for (unsigned int j = 0; j < loadTree.tree.at(a).w; j++){
							image2[loadTree.tree.at(a).z].at<cv::Vec3b>(loadTree.tree.at(a).y + i, loadTree.tree.at(a).x+j) = cv::Vec3b(0, 0, 0);
							//cout << n << endl;
						}
					}
			}
					...
					for (unsigned int i = 0; i < loadTree.tree.at(a).h; i++){
						for (unsigned int j = 0; j < loadTree.tree.at(a).w; j++){
							unsigned char x = test.data.at(n+i)[j];
							image2[loadTree.tree.at(a).z].at<cv::Vec3b>(loadTree.tree.at(a).y + i, loadTree.tree.at(a).x + j) = cv::Vec3b(x, x, x);
						}
					}
