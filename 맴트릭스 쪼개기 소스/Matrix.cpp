		...
		if (tree.tree.at(i).allBlack == false){
			...
			//	cout << tree.tree.at(i).x << " " << tree.tree.at(i).y << " " << tree.tree.at(i).z << " " << tree.tree.at(i).w << " " << tree.tree.at(i).h << " " << tree.tree.at(i).l<<" "<<endl;
				for (unsigned int j = 0; j < tree.tree.at(i).h; j++){
					data.push_back(new float[floatSize]);
					int id = data.size() - 1;
					for (unsigned int k = 0; k < floatSize; k++){
						if (k < tree.tree.at(i).w)
							...
							data.at(id)[k] = m.at<cv::Vec3f>(tree.tree.at(i).y + j, tree.tree.at(i).x + k, tree.tree.at(i).z)[0]; //여기다가.
						else
							data.at(id)[k] = 0;
					}
				} //cout << endl;
...
void Matrix::saveFile(ofstream &fout){
	...
	for (unsigned int i = 0; i < data.size(); i++){
		fout.write((char*)data.at(i), sizeof(float) * floatSize);
	}
}
...
void Matrix::openFile(ifstream &fin){
	...
	for (int i = 0; i < (int)dataSize; i++){
		//std::cout << i << endl;
		float* d;
		d = new float[floatSize];
		fin.read(reinterpret_cast<char *>(d), sizeof(float) * floatSize);
		data.push_back(d);
	}

	std::cout << "openFile ok " << endl;
