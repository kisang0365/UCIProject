...
int Tree::build(int x, int y, int z, int w, int h, int l, cv::Mat &m){
	...
	if (!node.needToSplite(x, y, z, w, h, l, m)){
		...
		for (int k = 0; k < l; k++){
			for (int i = 0; i < w; i++){
				m.at<cv::Vec3b>(y, x + i, z+k) = cv::Vec3b(0, 255, 0);
				m.at<cv::Vec3b>(y + h - 1, x + i,z+k) = cv::Vec3b(0, 255, 0);
			}
		}
		//col
		for (int k = 0; k < l; k++){
			for (int i = 0; i < h; i++){
				m.at<cv::Vec3b>(y + i, x, z+k) = cv::Vec3b(0, 255, 0);
				m.at<cv::Vec3b>(y + i, x + w - 1, z+k) = cv::Vec3b(0, 255, 0);
			}
		}
	}
	return id;
...
void Tree::makeB(unsigned int sizeX, unsigned int sizeY, unsigned int sizeZ, cv::Mat m){
	...
	for (unsigned int k = 0; k < sizeZ; k++){
		tempM[k] = cv::Mat(sizeY, sizeX, CV_8UC3, cv::Scalar(0));
		for (unsigned int i = 0; i < sizeY; i++){
			for (unsigned int j = 0; j < sizeX; j++){
				tempM[k].at<cv::Vec3b>(i, j) = m.at<cv::Vec3b>(i, j, k);
			}
		}
	}
		...
		if (tree.at(num).children[0] == -1){
			for (unsigned int k = 0; k < tree.at(num).l; k++){
				for (unsigned int i = 0; i < tree.at(num).w; i++){
					tempM[tree.at(num).z+k].at<cv::Vec3b>(tree.at(num).y, tree.at(num).x + i) = cv::Vec3b(0, 255, 0);
					tempM[tree.at(num).z+k].at<cv::Vec3b>(tree.at(num).y + tree.at(num).h - 1, tree.at(num).x + i) = cv::Vec3b(0, 255, 0);
